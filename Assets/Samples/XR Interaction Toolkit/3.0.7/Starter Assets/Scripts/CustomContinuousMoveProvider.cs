...
public class CustomContinuousMoveProvider : LocomotionProvider
{
    ...
    protected virtual void MoveRig(Vector3 translationInWorldSpace)
    {
        ...
        if (m_CharacterController != null && m_CharacterController.enabled)
        {
            ...
            if (Physics.SphereCast(new Ray(transform.position,Vector3.zero),0.05f,0f,_playerMask) || !m_UseGravity || m_EnableFly)
            {
